...
4. **`sp >= 1` live invariant is NOT enforced at serving time** — owner: **TBD**; fix: enforce `shortest_path >= 1` (or document an explicit, reviewed exception) at `live_graph_features.py:220`, where `self_illicit=True` currently yields `shortest_path=0` — the inverse of the snapshot's `sp >= 1` invariant. This is the training/serving asymmetry (live serving permits `{0..6}`, training only `{1..6}`); leaving it unaddressed means a future enrichment-set expansion could silently reintroduce the self-referential signal into live scoring. Impact is bounded today (only 2-3 enriched wallets exist), but the invariant must be enforced or the exception documented before the enrichment set grows.
