...
export function NetworkField() {
  ...
  useFrame(({ clock }, rawDelta) => {
    ...
    for (let i = 0; i < NODE_COUNT; i++) {
      const o = i * 3;
      const drift = Math.sin(t * 0.35 + i) * 0.12;
      const x =
        layoutDeep[o]! * wDeep +
      ...
      const isTron = ci === 2;
      const pulse = 0.75 + Math.sin(t * 2 + i * 0.7) * 0.25;
    ...
    for (let k = 0; k < LINKS.length; k++) {
      ...
      cB.multiplyScalar(strength * (0.6 + Math.sin(t + k) * 0.12));
    ...
    if (group.current) {
      px.current = damp(px.current, pointer.x * 1.6, 2.5, dt);
      py.current = damp(py.current, pointer.y * 1.1, 2.5, dt);
      group.current.position.x = -px.current;
      group.current.position.y = py.current;
      group.current.rotation.y =
        lerp(0, 0.5, smoothRange(p, 0.68, 1)) + px.current * 0.02;
