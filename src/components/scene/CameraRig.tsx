...
export function CameraRig({ reducedMotion = false }: CameraRigProps) {
  ...
  useFrame(({ camera }, rawDelta) => {
    ...
    const parallaxK = reducedMotion ? 0.2 : 1.0;
    const dampK = reducedMotion ? 7 : 4.5;
    const lookDampK = reducedMotion ? 6 : 4;

    mx.current = damp(
      mx.current,
      pointer.x * (reducedMotion ? 0.2 : 1.2),
      reducedMotion ? 3 : 1.8,
      dt,
    );
    my.current = damp(
      my.current,
      pointer.y * (reducedMotion ? 0.15 : 0.8),
      reducedMotion ? 3 : 1.8,
      dt,
    );
    pos.x += mx.current * parallaxK;
    pos.y -= my.current * parallaxK;

    cur.current.x = damp(cur.current.x, pos.x, dampK, dt);
    cur.current.y = damp(cur.current.y, pos.y, dampK, dt);
    cur.current.z = damp(cur.current.z, pos.z, dampK, dt);

    tgt.current.x = damp(tgt.current.x, look.x, lookDampK, dt);
    tgt.current.y = damp(tgt.current.y, look.y, lookDampK, dt);
    tgt.current.z = damp(tgt.current.z, look.z, lookDampK, dt);
