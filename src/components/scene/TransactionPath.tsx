...
interface TransactionPathProps {
  reducedMotion?: boolean;
}

export function TransactionPath({ reducedMotion = false }: TransactionPathProps) {
  ...
  useFrame(({ clock }) => {
    ...
    if (pulse.current) {
      ...
      const s = reducedMotion ? 0.28 : 0.28 + Math.sin(clock.elapsedTime * 5) * 0.05;
