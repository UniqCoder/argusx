...
export function BitcoinCoin({ reducedMotion = false }: BitcoinCoinProps) {
  ...
  useFrame((_, rawDelta) => {
    ...
    if (tilt.current) {
      const tiltMul = reducedMotion ? 0.25 : 1;
      mx.current = damp(mx.current, pointer.x * 0.18 * tiltMul, 3, dt);
      my.current = damp(my.current, pointer.y * 0.14 * tiltMul, 3, dt);
      tilt.current.rotation.y = mx.current + (reducedMotion ? 0 : Math.sin(p * 6) * 0.08);
